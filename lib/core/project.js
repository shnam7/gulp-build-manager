...
class GProject {
    ...
    addWatcher(config = { builder: 'watcher' }) {
        const gulp = require('gulp');
        const opts = utils_1.is.String(config) ? { name: config, builder: 'watcher' } : Object.assign({}, config);
        if (opts.browserSync)
    ...
    addCleaner(config = { builder: 'cleaner' }) {
        const opts = utils_1.is.String(config) ? { name: config, builder: 'cleaner' } : Object.assign({}, config);
        return this.addBuildItem({
            ...
            builder: (rtb) => {
                ...
                utils_1.msg(`[${rtb.conf.name}]: cleaning `, cleanList);

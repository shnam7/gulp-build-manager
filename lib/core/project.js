...
class GProject {
    ...
    addWatcher(config = { name: '@watcher' }, name = '@watch') {
        const gulp = require('gulp');
        const opts = utils_1.is.String(config) ? { name: config } : Object.assign({}, config, { name });
        if (opts.browserSync)
    ...
    addCleaner(config = { name: '@clean' }, name = '@clean') {
        const opts = utils_1.is.String(config) ? { name: config } : Object.assign({}, config, { name });
        return this.addBuildItem({

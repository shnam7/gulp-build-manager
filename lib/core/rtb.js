...
function toPromise(stream) {
    ...
    return utils_1.requireSafe('stream-to-promise')(stream);
}
//--- class RTB
...
class RTB extends events_1.EventEmitter {
    constructor(func) {
        ...
        this._buildFunc = (rtb) => { rtb.src().dest(); };
        this._conf = { buildName: '', buildOptions: {}, moduleOptions: {} };
        if (func)
    ...
    copy(param, options = {}) {
        ...
        const _copy = (target) => {
            ...
            if (options.verbose)
                utils_1.msg(`[${this.buildName}]:copying: ${copyInfo}`);
            return utils_1.copy(target.src, target.dest)
                .then(() => { if (this.conf.verbose)
                utils_1.msg(`[${this.buildName}]:copying: ${copyInfo} --> done`); });
        };
